...
async def tti_receive_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        # Use Gemini Flash image generation (free tier compatible)
        api_url = (
        ...
        if r.status_code != 200:
            ...
            await status_msg.edit_text(f"❌ Gemini API error ({r.status_code}).\nCheck GEMINI_API_KEY.")
            return ConversationHandler.END
        result = r.json()
        ...
        await status_msg.edit_text("❌ Timeout. Gemini took too long.")
    except Exception as e:
